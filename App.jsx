...
const AviatorIcon = ({ size = 56, rounded = true }) => (
  <div style={{
...
const numberToColor = (n) => (n === 0 || n === 5) ? "violet" : ([1,3,7,9].includes(n) ? "green" : "red");
const numberToSize  = (n) => (n <= 4 ? "small" : "big");
...
const AdminGameManagement = ({ showToast, onBack }) => {
...
const AdminDicePage = ({ showToast, onBack }) => {
...
const AdminAviatorPage = ({ showToast, onBack }) => {
...
// ─── AVIATOR GAME UTILS ────────────────────────────────────────────────────────
const generateCrashPoint = (avgTarget) => {
  const u = Math.random();
...
const AVIATOR_BETTING_DURATION = 6; // seconds to place bets before takeoff

// ─── AVIATOR GAME ───────────────────────────────────────────────────────────────
const AviatorGame = ({ user, setUser, setPage, showToast }) => {
  const cfg = DB.get("dp_platform_config") || {};
  const COST = cfg.gameCost || 5;
