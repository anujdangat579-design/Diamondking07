...
**What's included now:**
- `SecurityGuard` (in `src/core.jsx`, mounted in `App.jsx`) — blocks right-click, blocks F12/Ctrl+Shift+I/Ctrl+U shortcuts, and shows a console warning if devtools looks open.
- These are **casual-user deterrents only**. They do not stop a real attacker — devtools can still be opened via browser menu, mobile devtools, or external tools like Postman/curl, and none of that requires touching your website at all.

**The real risk — read this:**
`firestore.rules` currently has `allow read, write: if true` on the entire database. Since your Firebase project config (project ID, API key) is embedded in the JS bundle that ships to every browser, **anyone can call the Firestore API directly** — bypassing your website, your React code, and any F12 blocking entirely — to:
- give themselves unlimited diamonds
- fake a deposit as "approved"
- change game odds / force-win config
- delete other users' transactions

This is possible today regardless of any client-side protection, because there's no way for Firestore to tell "your real app" apart from a direct API call.

**Real fixes, in order of effort:**
1. **Firebase App Check** (few hours) — verifies requests come from your actual deployed app (via reCAPTCHA v3), blocking casual scripted attacks. Doesn't stop a skilled attacker who inspects your app, but raises the bar a lot. Enable in Firebase Console → App Check.
2. **Firebase Authentication + per-user rules** (bigger lift) — replace the custom phone/password login with real Firebase Auth, then write rules like `allow write: if request.auth.uid == userId` so users can only touch their own record.
3. **Cloud Functions for money-moving actions** (most important for a real-money app) — deposit approval, withdrawal processing, and admin force-win controls should happen in a server-side Cloud Function, never as a direct client write. This is the only way to make balance changes actually trustworthy.
