...
export const S = {
  ...
  page: { paddingBottom: 90, minHeight: "100vh", width: "100%", maxWidth: "100vw", overflowX: "hidden" },
...
// Admin-controlled promotional banner — what it promotes (another app,
// channel, partner link, anything) is entirely up to whatever the admin
// sets in Settings → Promo. Renders nothing if there's no active banner.
export const PromoBanner = ({ banner }) => {
  if (!banner || !banner.active || !(banner.title || banner.imageUrl)) return null;
  const open = () => { if (banner.link) window.open(banner.link, "_blank", "noopener,noreferrer"); };
  return (
    <div onClick={open} style={{
      display: "flex", alignItems: "center", gap: 12, marginBottom: 14, padding: 12, borderRadius: 16,
      background: banner.imageUrl ? "rgba(255,255,255,0.03)" : "linear-gradient(135deg, rgba(0,212,255,0.1), rgba(181,55,242,0.1))",
      border: `1px solid ${S.neonGold}33`, cursor: banner.link ? "pointer" : "default", overflow: "hidden",
    }}>
      {banner.imageUrl && (
        <img src={banner.imageUrl} alt="" style={{ width: 52, height: 52, borderRadius: 12, objectFit: "cover", flexShrink: 0 }} onError={e => { e.target.style.display = "none"; }} />
      )}
      <div style={{ flex: 1, minWidth: 0 }}>
        {banner.title && <div style={{ fontWeight: 800, fontSize: 13.5, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{banner.title}</div>}
        {banner.subtitle && <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.5)", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{banner.subtitle}</div>}
      </div>
      {banner.buttonText && (
        <div style={{ flexShrink: 0, background: S.gradBlue, color: "#fff", fontWeight: 800, fontSize: 11.5, borderRadius: 99, padding: "7px 14px", whiteSpace: "nowrap" }}>
          {banner.buttonText}
        </div>
      )}
    </div>
