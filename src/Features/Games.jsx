...
export const ensureQuizDataSeeded = () => {
  ...
  if (!DB.get("dp_quiz_questions")) {
    DB.set("dp_quiz_questions", QUIZ_QUESTION_BANK.map(q => ({
      ...
      options: q.options, correct: q.correct, active: true, createdAt: new Date().toISOString(),
    })));
  }
};
...
const pickQuizQuestions = () => {
  ...
  for (let i = 0; i < QUIZ_QUESTIONS_PER_BATTLE && pool.length; i++) {
    ...
    picked.push({ subject: dbQ.categoryId, q: dbQ.question, options: dbQ.options, correct: dbQ.correct });
  }
  return picked;
};
...
// Admin-only: force-cancel a stuck waiting/active battle and refund both
// entry fees. Used from Match Monitoring when a battle looks abandoned.
