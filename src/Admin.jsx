...
import { ensureQuizDataSeeded, adminCancelQuizBattle } from "../features/Games.jsx";
...
export const AdminQuizQuestions = ({ showToast, onBack }) => {
  ...
  const blankForm = { id: null, categoryId: cats[0]?.id || "", question: "", options: ["", "", "", ""], correct: 0, active: true };
  const [form, setForm] = useState(blankForm);

  const save = (list) => { DB.set("dp_quiz_questions", list); setQuestions(list); };

  const openAdd = () => { setForm(blankForm); setEditing("new"); };
  const openEdit = (q) => { setForm({ ...q, options: [...q.options] }); setEditing(q.id); };
  ...
  const submitForm = () => {
    ...
    if (editing === "new") {
      const q = { id: tid(), categoryId: form.categoryId, question: form.question.trim(), options: form.options.map(o => o.trim()), correct: form.correct, active: true, createdAt: new Date().toISOString() };
      save([q, ...questions]);
      showToast("✅ Question add ho gaya", "success");
    } else {
      save(questions.map(q => q.id === editing ? { ...q, categoryId: form.categoryId, question: form.question.trim(), options: form.options.map(o => o.trim()), correct: form.correct } : q));
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        {shown.map(q => (
          <Card key={q.id} style={{ marginBottom: 8, opacity: q.active ? 1 : 0.5 }}>
            <div style={{ fontSize: 11, color: S.neonGold, fontWeight: 700, marginBottom: 4 }}>{catName(q.categoryId)}</div>
