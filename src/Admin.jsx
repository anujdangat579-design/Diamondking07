...
import { ensureQuizDataSeeded, adminCancelQuizBattle, getQuizDailyStats, QUIZ_HARD_MODE_THRESHOLD, QUIZ_HARD_PACK, adminImportHardPack, extractTextFromPdf, parseQuizPdfText, getDailyQuizPool } from "../features/Games.jsx";
...
import { settleCustomRound } from "../features/Games.jsx";
...
export const AdminGameManagement = ({ showToast, onBack }) => {
  ...
  const logResult = (game, resultLabel, mode) => {
    ...
    settleCustomRound(game, resultLabel); // pays out/loses every bet on this round, then starts the next round
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        {/* Quiz Battle — Categories / Question Bank / Match Monitoring */}
        <Card onClick={() => setPage("admin_quiz")} style={{ marginBottom: 16, background: "rgba(255,215,0,0.08)", border: `1px solid ${S.neonGold}44` }} glow>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <div style={{ fontSize: 28 }}>🧠</div>
              <div>
                <div style={{ fontWeight: 800 }}>Quiz Battle</div>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>{(DB.get("dp_quiz_questions") || []).length} questions · Categories, Question Bank, Match Monitoring</div>
              </div>
            </div>
...
export const AdminOverview = ({ setPage, onLogout }) => {
  ...
  const secLogs      = DB.get("dp_security_logs") || [];
    ...
    <div style={S.page}>
      ...
      <div style={{ padding:"0 16px 24px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:16 }}>
          ...
          <div onClick={()=>setPage("admin_giftcodes")} style={{ background:"rgba(255,61,154,0.07)", border:`1px solid ${S.neonPink}33`, borderRadius:16, padding:"14px 14px", cursor:"pointer" }}>
            <div style={{ fontSize:22, marginBottom:6 }}>🎁</div>
            <div style={{ fontWeight:800, fontSize:13 }}>Gift Codes</div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)" }}>Create redeem codes →</div>
          </div>
          <div onClick={()=>setPage("admin_announcements")} style={{ background:"rgba(0,212,255,0.07)", border:`1px solid ${S.neonBlue}33`, borderRadius:16, padding:"14px 14px", cursor:"pointer" }}>
            <div style={{ fontSize:22, marginBottom:6 }}>📢</div>
            <div style={{ fontWeight:800, fontSize:13 }}>Announcements</div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)" }}>Post notices to all users →</div>
          </div>
          <div onClick={()=>setPage("admin_security")} style={{ background:"rgba(255,61,154,0.08)", border:"1.5px solid rgba(255,61,154,0.3)", borderRadius:16, padding:"14px 14px", cursor:"pointer", position:"relative" }}>
            <div style={{ fontSize:22, marginBottom:6 }}>🔒</div>
            <div style={{ fontWeight:800, fontSize:13, color:S.neonPink }}>Security</div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)" }}>Fraud · Anti-hack →</div>
          </div>
        </div>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        {/* ── VIP LEVELS ── */}
        {tab === "vip" && <VIPLevelsManagement cfg={cfg} setCfg={setCfg} saveCfg={saveCfg} />}

        {/* ── PROMO BANNER ── */}
        {tab === "promo" && <PromoBannerManagement cfg={cfg} setCfg={setCfg} saveCfg={saveCfg} showToast={showToast} />}
...
// ─── VIP LEVELS MANAGEMENT (Admin Settings → VIP tab) ────────────────────────
// Edits the deposit thresholds, daily bonus and rebate % for each VIP tier.
// Users' levels are computed live from their totalDeposited — nothing to
// migrate here, changing a threshold just moves users into/out of a tier.
export const VIPLevelsManagement = ({ cfg, setCfg, saveCfg }) => {
  const levels = cfg.vipLevels || [];

  const updateLevel = (level, field, val) => {
    setCfg(c => ({
      ...c,
      vipLevels: (c.vipLevels || []).map(lv => lv.level === level ? { ...lv, [field]: Number(val) } : lv),
    }));
  };
    ...
    <div>
      <Card style={{ background: "rgba(0,212,255,0.05)", marginBottom: 14 }}>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>
          💡 A user's VIP level is based on their lifetime deposit total — set each tier's minimum deposit, daily claimable bonus, and rebate % below.
        </div>
      </Card>

      {levels.map(lv => (
        <Card key={lv.level} style={{ marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 20 }}>{levelIcon[lv.level] || "🛡️"}</span>
            <span style={{ fontWeight: 800, fontSize: 14 }}>{lv.name}</span>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            <Input label="Min Deposit ₹" value={String(lv.minDeposit)} onChange={v => updateLevel(lv.level, "minDeposit", v)} type="number" />
            <Input label="Daily Bonus 💎" value={String(lv.dailyBonus)} onChange={v => updateLevel(lv.level, "dailyBonus", v)} type="number" />
            <Input label="Rebate %" value={String(lv.rebatePercent)} onChange={v => updateLevel(lv.level, "rebatePercent", v)} type="number" />
          </div>
        </Card>
      ))}

      <Btn full variant="green" onClick={saveCfg}>💾 Save VIP Levels</Btn>
    </div>
  );
};

// ─── PROMO BANNER MANAGEMENT (Admin Settings → Promo tab) ────────────────────
// Controls the promotional banner shown above the Leaderboard. Admin decides
// exactly what's being promoted (another app, a channel, a partner, anything)
// — nothing here is hardcoded, it's fully editable and can be turned off.
export const PromoBannerManagement = ({ cfg, setCfg, saveCfg, showToast }) => {
  const banner = cfg.promoBanner || {};
  const update = (field, val) => setCfg(c => ({ ...c, promoBanner: { ...(c.promoBanner || {}), [field]: val } }));

  const save = () => {
    if (banner.active && !((banner.title || "").trim())) { showToast("Active karne se pehle title daalo", "error"); return; }
    if (banner.active && !((banner.link || "").trim())) { showToast("Active karne se pehle link daalo", "error"); return; }
    saveCfg();
  ...
  return (
    <div>
      <Card style={{ background: "rgba(0,212,255,0.05)", marginBottom: 14 }}>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>
          💡 Leaderboard page ke top pe ek banner dikhta hai — jo app/channel/link promote karna hai wo yahan set karo. Tap karne par `link` naye tab mein khulega.
        </div>
      </Card>

      <Card style={{ marginBottom: 14 }}>
        <div onClick={() => update("active", !banner.active)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: 14 }}>Banner Active</div>
            <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>Off karne par Leaderboard pe kuch nahi dikhega</div>
          </div>
          <button style={{ width: 48, height: 26, borderRadius: 13, background: banner.active ? S.neonGreen : "rgba(255,255,255,0.15)", border: "none", cursor: "pointer", position: "relative" }}>
            <div style={{ position: "absolute", top: 3, left: banner.active ? 25 : 3, width: 20, height: 20, borderRadius: "50%", background: "#fff", transition: "left 0.2s" }} />
          </button>
        </div>
      </Card>

      <Input label="Title" placeholder="e.g. Try our new Rummy app!" value={banner.title || ""} onChange={v => update("title", v)} icon="📌" />
      <Input label="Subtitle (optional)" placeholder="e.g. Download now & get ₹50 free" value={banner.subtitle || ""} onChange={v => update("subtitle", v)} icon="✏️" />
      <Input label="Button Text" placeholder="e.g. Install Now" value={banner.buttonText || ""} onChange={v => update("buttonText", v)} icon="🔘" />
      <Input label="Link (opens on tap)" placeholder="https://..." value={banner.link || ""} onChange={v => update("link", v)} icon="🔗" />
      <Input label="Image URL (optional)" placeholder="https://... (leave blank for a text-only banner)" value={banner.imageUrl || ""} onChange={v => update("imageUrl", v)} icon="🖼️" />

      {/* Live preview, same layout as what users will see */}
      {banner.active && (banner.title || banner.imageUrl) && (
        <>
          <div style={{ fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,0.4)", letterSpacing: 1, margin: "14px 0 8px" }}>PREVIEW</div>
          <PromoBanner banner={banner} />
        </>
      )}

      <Btn full variant="green" onClick={save} style={{ marginTop: 10 }}>💾 Save Promo Banner</Btn>
    </div>
  );
};
...
// ─── GIFT / REDEEM CODES ──────────────────────────────────────────────────────
// Admin creates a code with a diamond value + max redemptions (or unlimited)
// and optional expiry. Users redeem it once each from Profile → Redeem Code.
export const AdminGiftCodes = ({ showToast, onBack }) => {
  const [codes, setCodes] = useState(DB.get("dp_gift_codes") || []);
  const [form, setForm] = useState({ code: "", diamonds: 100, maxUses: 100, expiresAt: "" });

  const save = (list) => { DB.set("dp_gift_codes", list); setCodes(list); };

  const createCode = () => {
    const code = (form.code || "").trim().toUpperCase();
    if (!code) { showToast("Code enter karo", "error"); return; }
    if (codes.some(c => c.code === code)) { showToast("Yeh code already exist karta hai", "error"); return; }
    const newCode = {
      id: tid(), code, diamonds: Number(form.diamonds) || 0,
      maxUses: Number(form.maxUses) || 0, // 0 = unlimited
      usedBy: [], expiresAt: form.expiresAt || null,
      createdAt: new Date().toISOString(), active: true,
    };
    save([newCode, ...codes]);
    setForm({ code: "", diamonds: 100, maxUses: 100, expiresAt: "" });
    showToast(`✅ Gift code ${code} created!`, "success");
  };

  const toggleActive = (id) => save(codes.map(c => c.id === id ? { ...c, active: !c.active } : c));
  const deleteCode = (id) => save(codes.filter(c => c.id !== id));

  return (
    <div style={S.page}>
      <TopBar title="🎁 Gift Codes" onBack={onBack} />
      <div style={{ padding: "0 20px" }}>
        <Card style={{ marginBottom: 16 }}>
          <div style={{ fontWeight: 800, marginBottom: 10 }}>+ Create New Code</div>
          <Input label="Code (e.g. DIWALI2026)" value={form.code} onChange={v => setForm(f => ({ ...f, code: v.toUpperCase() }))} icon="🔖" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <Input label="Diamonds 💎" value={String(form.diamonds)} onChange={v => setForm(f => ({ ...f, diamonds: v }))} type="number" />
            <Input label="Max Uses (0=∞)" value={String(form.maxUses)} onChange={v => setForm(f => ({ ...f, maxUses: v }))} type="number" />
          </div>
          <Btn full variant="green" onClick={createCode}>🎁 Create Code</Btn>
        </Card>

        <div style={{ fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,0.4)", letterSpacing: 1, marginBottom: 8 }}>ALL CODES ({codes.length})</div>
        {codes.length === 0 && <Card style={{ textAlign: "center", padding: 24, color: "rgba(255,255,255,0.3)" }}>Abhi koi code nahi banaya</Card>}
        {codes.map(c => (
          <Card key={c.id} style={{ marginBottom: 8, opacity: c.active ? 1 : 0.5 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 900, fontSize: 15, letterSpacing: 1 }}>{c.code}</div>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginTop: 2 }}>
                  💎{c.diamonds} · {c.usedBy.length}/{c.maxUses || "∞"} used
                </div>
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                <Btn sm variant={c.active ? "ghost" : "green"} onClick={() => toggleActive(c.id)}>{c.active ? "Pause" : "Resume"}</Btn>
                <Btn sm variant="danger" onClick={() => deleteCode(c.id)}>🗑️</Btn>
              </div>
            </div>
          </Card>
        ))}
...
export const AdminAnnouncements = ({ showToast, onBack }) => {
  const [items, setItems] = useState(DB.get("dp_announcements") || []);
  const [form, setForm] = useState({ title: "", body: "" });

  const save = (list) => { DB.set("dp_announcements", list); setItems(list); };

  const post = () => {
    if (!form.title.trim() || !form.body.trim()) { showToast("Title aur message dono bharo", "error"); return; }
    const item = { id: tid(), title: form.title.trim(), body: form.body.trim(), createdAt: new Date().toISOString(), pinned: false };
    save([item, ...items]);
    setForm({ title: "", body: "" });
  ...
  return (
    <div style={S.page}>
      <TopBar title="📢 Announcements" onBack={onBack} />
      <div style={{ padding: "0 20px" }}>
        <Card style={{ marginBottom: 16 }}>
          <div style={{ fontWeight: 800, marginBottom: 10 }}>+ New Announcement</div>
          <Input label="Title" value={form.title} onChange={v => setForm(f => ({ ...f, title: v }))} icon="📌" />
          <div style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 4 }}>Message</div>
            <textarea
              value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))}
              rows={3}
              style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, color: "#fff", padding: 10, fontSize: 13, resize: "vertical", boxSizing: "border-box" }}
            />
          </div>
          <Btn full variant="green" onClick={post}>📢 Post Announcement</Btn>
        </Card>

        <div style={{ fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,0.4)", letterSpacing: 1, marginBottom: 8 }}>ALL ANNOUNCEMENTS ({items.length})</div>
        {items.length === 0 && <Card style={{ textAlign: "center", padding: 24, color: "rgba(255,255,255,0.3)" }}>Abhi koi announcement nahi</Card>}
        {items.sort((a,b) => (b.pinned - a.pinned) || (b.createdAt.localeCompare(a.createdAt))).map(i => (
          <Card key={i.id} style={{ marginBottom: 8, border: i.pinned ? `1px solid ${S.neonGold}55` : undefined }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: 14 }}>{i.pinned && "📌 "}{i.title}</div>
                <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.5)", marginTop: 4, whiteSpace: "pre-wrap" }}>{i.body}</div>
                <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.3)", marginTop: 6 }}>{timeAgo(i.createdAt)}</div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <Btn sm variant="ghost" onClick={() => togglePin(i.id)}>{i.pinned ? "Unpin" : "Pin"}</Btn>
                <Btn sm variant="danger" onClick={() => remove(i.id)}>🗑️</Btn>
              </div>
            </div>
          </Card>
        ))}
...
// ═══════════════════════════════════════════════════════════════════════════════
export const AdminSecurity = ({ showToast, onBack }) => {
  const [tab,    setTab]    = useState("logs");    // logs | devices | rules | audit
  const [search, setSearch] = useState("");
  const [tick,   setTick]   = useState(0);
  ...
  // ── Stats ──
  const blockedUsers   = allUsers.filter(u => u.banned).length;
  const suspiciousLogs = secLogs.filter(l => !l.reviewed).length;
  const multiAccounts  = (() => {
    const deviceMap = {};
    allUsers.forEach(u => { if (u.deviceId) { deviceMap[u.deviceId] = (deviceMap[u.deviceId]||[]); deviceMap[u.deviceId].push(u); } });
    return Object.values(deviceMap).filter(arr => arr.length > 1).length;
  })();
  const todayLogins = loginLogs.filter(l => l.time >= new Date(Date.now()-86400000).toISOString()).length;

  // ── Actions ──
  const markReviewed = (id) => {
    const updated = secLogs.map(l => l.id===id ? {...l,reviewed:true} : l);
    DB.set("dp_security_logs", updated);
    setTick(t=>t+1);
  };

  const blockUser = (userId, reason) => {
    const users = DB.get("dp_users") || [];
    DB.set("dp_users", users.map(u => u.id===userId ? {...u,banned:true,banReason:reason,bannedAt:new Date().toISOString()} : u));
  ...
  const unblockUser = (userId) => {
    const users = DB.get("dp_users") || [];
    DB.set("dp_users", users.map(u => u.id===userId ? {...u,banned:false,banReason:null} : u));
  ...
  const clearOldLogs = () => {
    ...
    DB.set("dp_security_logs", secLogs.filter(l=>l.time>=cutoff));
    showToast("Old logs cleared","info");
    setTick(t=>t+1);
  };

  // ── Log type styles ──
  const logTypeStyle = {
    large_bet:   { color: "#ffd700", icon: "💰", label: "Large Bet" },
    win_streak:  { color: "#ff6b6b", icon: "🎰", label: "Win Streak" },
    multi_login: { color: "#ff9d3d", icon: "📱", label: "Multi Login" },
    fast_bet:    { color: "#ff3d9a", icon: "⚡", label: "Fast Bet" },
    device_swap: { color: "#b537f2", icon: "🔄", label: "Device Swap" },
    suspicious:  { color: "#ff6b6b", icon: "⚠️", label: "Suspicious" },
  };

  const getUserName = (id) => allUsers.find(u=>u.id===id)?.name || id?.slice(0,8) || "Unknown";

  // ── Device groups (same device → multiple accounts) ──
  const deviceGroups = (() => {
    const map = {};
    allUsers.forEach(u => {
      if (!u.isAdmin) {
        const key = u.deviceId || u.ipAddress || "unknown";
        if (!map[key]) map[key] = [];
        map[key].push(u);
      }
    });
    return Object.entries(map).filter(([,arr])=>arr.length>1).sort((a,b)=>b[1].length-a[1].length);
  })();

  // ── Security Rules ──
  const cfg = DB.get("dp_platform_config") || {};
  const [maxBetPct,    setMaxBetPct]    = useState(cfg.maxBetPct    || 80);
  const [minBetInterval,setMinBetInt]   = useState(cfg.minBetInterval|| 800);
  const [maxBetsPerMin, setMaxBetsMin]  = useState(cfg.maxBetsPerMin || 30);
  const [autoBlockWins, setAutoBlockW]  = useState(cfg.autoBlockWins || 10);
  const [reqKYCAbove,   setReqKYCAbove] = useState(cfg.reqKYCAbove   || 5000);

  const saveRules = () => {
    DB.set("dp_platform_config", {...cfg, maxBetPct, minBetInterval, maxBetsPerMin, autoBlockWins, reqKYCAbove});
    showToast("✅ Security rules saved","success");
  };

  const filteredLogs = secLogs.filter(l => !search || getUserName(l.userId).toLowerCase().includes(search.toLowerCase()) || l.type.includes(search));
  const filteredLogins = loginLogs.filter(l => !search || l.phone?.includes(search)).slice(0,50);

  return (
    <div style={{...S.page, background:"#080818"}}>
      {/* Header */}
      <div style={{background:"linear-gradient(180deg,#1a0a3e,transparent)",padding:"14px 20px 10px",position:"sticky",top:0,zIndex:50,backdropFilter:"blur(12px)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          <button onClick={onBack} style={{width:34,height:34,borderRadius:"50%",background:"rgba(255,255,255,0.08)",border:"none",color:"#fff",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
          <div style={{flex:1}}>
            <div style={{fontSize:18,fontWeight:900}}>🔒 Security Center</div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.45)"}}>Fraud detection · Anti-hack · User safety</div>
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:12}}>
          {[
            ["⚠️",suspiciousLogs,"Alerts",S.neonPink],
            ["🚫",blockedUsers,"Blocked",S.neonOrange],
            ["📱",multiAccounts,"Multi Acc",S.neonGold],
            ["🔓",todayLogins,"Today Login",S.neonBlue],
          ].map(([ic,v,l,c])=>(
            <div key={l} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"8px 6px",textAlign:"center"}}>
              <div style={{fontWeight:900,fontSize:16,color:c}}>{v}</div>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",marginTop:1}}>{ic} {l}</div>
            </div>
          ))}
        </div>

        {/* Tabs */}
        <div style={{display:"flex",gap:4,background:"rgba(255,255,255,0.05)",borderRadius:12,padding:4,overflowX:"auto"}}>
          {[["logs","⚠️ Alerts"],["devices","📱 Devices"],["logins","🔓 Logins"],["rules","⚙️ Rules"]].map(([k,l])=>(
            <button key={k} onClick={()=>setTab(k)} style={{flex:"0 0 auto",padding:"7px 12px",borderRadius:9,border:"none",cursor:"pointer",fontWeight:700,fontSize:11,whiteSpace:"nowrap",background:tab===k?S.gradBlue:"transparent",color:tab===k?"#fff":"rgba(255,255,255,0.45)",transition:"all 0.2s"}}>{l}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{padding:"10px 16px 100px"}}>
        ...
        <div style={{position:"relative",marginBottom:12}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search user / type..."
            style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:12,padding:"10px 14px 10px 36px",color:"#fff",fontSize:13,outline:"none",boxSizing:"border-box"}}/>
        </div>
        ...
        {tab==="logs" && (
          <>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>{filteredLogs.length} security events</div>
              <button onClick={clearOldLogs} style={{background:"none",border:"none",color:S.neonBlue,fontSize:12,cursor:"pointer",fontWeight:700}}>🗑 Clear Old</button>
            </div>
            ...
            {filteredLogs.map(log=>{
              const sty = logTypeStyle[log.type] || logTypeStyle.suspicious;
              const u   = allUsers.find(x=>x.id===log.userId);
              return (
                <div key={log.id} style={{background:log.reviewed?"rgba(255,255,255,0.03)":`rgba(255,61,154,0.07)`,border:`1px solid ${log.reviewed?"rgba(255,255,255,0.06)":sty.color+"44"}`,borderRadius:14,padding:"12px 14px",marginBottom:10}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                    <div style={{display:"flex",gap:10,alignItems:"center"}}>
                      <div style={{width:36,height:36,borderRadius:10,background:`${sty.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{sty.icon}</div>
                      <div>
                        <div style={{fontWeight:700,fontSize:13,color:sty.color}}>{sty.label}</div>
                        <div style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>{getUserName(log.userId)} · {timeAgo(log.time)}</div>
                      </div>
                    </div>
                    {!log.reviewed && <div style={{width:8,height:8,borderRadius:"50%",background:S.neonPink,flexShrink:0,marginTop:4}}/>}
                  </div>
                  <div style={{fontSize:12,color:"rgba(255,255,255,0.6)",marginBottom:10,background:"rgba(255,255,255,0.04)",borderRadius:8,padding:"6px 10px"}}>{log.details}</div>
                  <div style={{display:"flex",gap:8}}>
                    {!log.reviewed && <button onClick={()=>markReviewed(log.id)} style={{flex:1,padding:"7px",borderRadius:10,border:"1px solid rgba(0,255,136,0.3)",background:"rgba(0,255,136,0.08)",color:S.neonGreen,fontWeight:700,fontSize:12,cursor:"pointer"}}>✓ Mark Safe</button>}
                    {u && !u.banned && <button onClick={()=>blockUser(log.userId,"Suspicious activity: "+log.type)} style={{flex:1,padding:"7px",borderRadius:10,border:"1px solid rgba(255,61,154,0.3)",background:"rgba(255,61,154,0.08)",color:S.neonPink,fontWeight:700,fontSize:12,cursor:"pointer"}}>🚫 Block User</button>}
                    {u && u.banned && <button onClick={()=>unblockUser(log.userId)} style={{flex:1,padding:"7px",borderRadius:10,border:"1px solid rgba(0,255,136,0.3)",background:"rgba(0,255,136,0.08)",color:S.neonGreen,fontWeight:700,fontSize:12,cursor:"pointer"}}>✓ Unblock</button>}
                  </div>
                </div>
              );
            })}
        ...
        {tab==="devices" && (
          <>
            <div style={{fontSize:12,color:"rgba(255,255,255,0.4)",marginBottom:12}}>
              Same device / IP = multiple accounts detected. Review & block if suspicious.
            </div>
            {deviceGroups.length===0 && (
              <div style={{textAlign:"center",padding:40,color:"rgba(255,255,255,0.3)"}}>
                <div style={{fontSize:40,marginBottom:8}}>✅</div>
                <div>No multi-account devices found</div>
              </div>
            )}
            {deviceGroups.map(([deviceId, users])=>(
              <Card key={deviceId} style={{marginBottom:12,border:"1px solid rgba(255,215,0,0.2)"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:13,color:S.neonGold}}>📱 {deviceId.slice(0,20)}...</div>
                    <div style={{fontSize:11,color:"rgba(255,255,255,0.4)"}}>{users.length} accounts on same device</div>
                  </div>
                  <Badge label={`${users.length} accts`} color={S.neonPink}/>
                </div>
                {users.map(u=>(
                  <div key={u.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderTop:"1px solid rgba(255,255,255,0.05)"}}>
                    <div>
                      <div style={{fontWeight:600,fontSize:13}}>{u.name}</div>
                      <div style={{fontSize:11,color:"rgba(255,255,255,0.4)"}}>📱 {u.phone} · 💎{u.diamonds}</div>
                    </div>
                    {u.banned
                      ? <button onClick={()=>unblockUser(u.id)} style={{padding:"5px 10px",borderRadius:8,border:"1px solid rgba(0,255,136,0.3)",background:"rgba(0,255,136,0.08)",color:S.neonGreen,fontWeight:700,fontSize:11,cursor:"pointer"}}>Unblock</button>
                      : <button onClick={()=>blockUser(u.id,"Multi account")} style={{padding:"5px 10px",borderRadius:8,border:"1px solid rgba(255,61,154,0.3)",background:"rgba(255,61,154,0.08)",color:S.neonPink,fontWeight:700,fontSize:11,cursor:"pointer"}}>🚫 Block</button>
                    }
                  </div>
                ))}
              </Card>
        ...
        {tab==="logins" && (
          <>
            <div style={{fontSize:12,color:"rgba(255,255,255,0.4)",marginBottom:12}}>Recent login attempts — all users</div>
            ...
            {filteredLogins.map((log,i)=>{
              const u = allUsers.find(x=>x.phone===log.phone||x.id===log.userId);
              return (
                <div key={i} style={{display:"flex",gap:12,alignItems:"center",padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
                  <div style={{width:34,height:34,borderRadius:"50%",background:log.success?"rgba(0,255,136,0.1)":"rgba(255,61,154,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>
                    {log.success?"🔓":"❌"}
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:600,fontSize:13}}>{u?.name||log.phone||"Unknown"}</div>
                    <div style={{fontSize:11,color:"rgba(255,255,255,0.4)"}}>
                      {log.ip||"IP not recorded"} · {timeAgo(log.time)}
                    </div>
                  </div>
                  <Badge label={log.success?"Login":"Failed"} color={log.success?S.neonGreen:"#ff6b6b"}/>
                </div>
              );
            })}
          </>
        ...
        {tab==="rules" && (
          <>
            <Card style={{marginBottom:14,background:"rgba(255,61,154,0.05)",border:"1px solid rgba(255,61,154,0.2)"}}>
              <div style={{fontWeight:800,fontSize:14,marginBottom:14}}>🛡️ Anti-Cheat Rules</div>

              {[
                {label:"Max bet % of balance",sub:"Block bets exceeding this % of user balance",val:maxBetPct,set:setMaxBetPct,unit:"%",min:10,max:100},
                {label:"Min bet interval (ms)",sub:"Minimum gap between bets (anti-rapid-fire)",val:minBetInterval,set:setMinBetInt,unit:"ms",min:200,max:5000},
                {label:"Max bets per minute",sub:"Auto-flag if user places more than this many bets/min",val:maxBetsPerMin,set:setMaxBetsMin,unit:"bets",min:5,max:100},
                {label:"Auto-flag win streak",sub:"Alert when user wins more than X times in 5 min",val:autoBlockWins,set:setAutoBlockW,unit:"wins",min:3,max:30},
                {label:"Require KYC above (₹)",sub:"Force KYC verification before withdrawal above this amount",val:reqKYCAbove,set:setReqKYCAbove,unit:"₹",min:1000,max:100000},
              ].map(rule=>(
                <div key={rule.label} style={{marginBottom:14}}>
                  <div style={{fontWeight:700,fontSize:13,marginBottom:2}}>{rule.label}</div>
                  <div style={{fontSize:11,color:"rgba(255,255,255,0.4)",marginBottom:6}}>{rule.sub}</div>
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    <input type="number" value={rule.val} onChange={e=>rule.set(Math.max(rule.min,Math.min(rule.max,parseInt(e.target.value)||rule.min)))}
                      style={{flex:1,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:10,padding:"9px 12px",color:"#fff",fontSize:14,outline:"none"}}/>
                    <div style={{fontSize:13,color:"rgba(255,255,255,0.4)",fontWeight:600,minWidth:36}}>{rule.unit}</div>
                  </div>
                </div>
              ))}

              <button onClick={saveRules} style={{width:"100%",padding:"13px",borderRadius:14,border:"none",background:S.gradBlue,color:"#fff",fontWeight:800,fontSize:14,cursor:"pointer",boxShadow:"0 4px 16px rgba(0,212,255,0.3)"}}>
                💾 Save Security Rules
              </button>
            </Card>

            <Card style={{background:"rgba(255,215,0,0.04)",border:"1px solid rgba(255,215,0,0.15)"}}>
              <div style={{fontWeight:800,fontSize:13,marginBottom:10}}>🔒 Active Protections</div>
              {[
                ["✅","Rate limiting","Max 10 bets per 5 seconds per user"],
                ["✅","Bet gap enforce","Min 800ms between consecutive bets"],
                ["✅","Large bet alert","Auto-flag bets >80% of balance"],
                ["✅","Win streak detect","Flag 8+ consecutive wins in 5 min"],
                ["✅","Multi-account detect","Same device ID = flagged"],
                ["✅","Admin result control","Forced outcome clears after 1 round"],
                ["✅","Firebase sync","All bets verified server-side via Firestore"],
                ["✅","Session lock","Each session tied to device fingerprint"],
              ].map(([ic,l,d])=>(
                <div key={l} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
...
// ─── QUIZ BATTLE ADMIN HUB ────────────────────────────────────────────────────
export const AdminQuizHub = ({ setPage, showToast }) => {
  ensureQuizDataSeeded();
  const cats = DB.get("dp_quiz_categories") || [];
  const questions = DB.get("dp_quiz_questions") || [];
  const battles = DB.get("dp_quiz_battles") || [];
  const liveCount = battles.filter(b => b.status === "waiting" || b.status === "active").length;
  const dailyStats = getQuizDailyStats();
  const [thresholdInput, setThresholdInput] = useState(String(dailyStats.threshold));
  const cfg0 = DB.get("dp_platform_config") || {};
  const [poolSizeInput, setPoolSizeInput] = useState(String(cfg0.quizDailyPoolSize || 150));
  const todaysPool = getDailyQuizPool(questions.filter(q => q.active));

  const saveThreshold = () => {
    const n = parseInt(thresholdInput, 10);
    if (!n || n < 1) { showToast("Valid number daalo", "error"); return; }
    const cfg = DB.get("dp_platform_config") || {};
    DB.set("dp_platform_config", { ...cfg, quizHardThreshold: n });
    showToast(`✅ Hard mode threshold ${n} set ho gaya`, "success");
  };

  const savePoolSize = () => {
    const n = parseInt(poolSizeInput, 10);
    if (!n || n < 5) { showToast("Kam se kam 5 daalo", "error"); return; }
    const cfg = DB.get("dp_platform_config") || {};
    DB.set("dp_platform_config", { ...cfg, quizDailyPoolSize: n });
    showToast(`✅ Daily pool size ${n} set ho gaya`, "success");
  };

  const Row = ({ icon, title, desc, page: p, color }) => (
    <Card onClick={() => setPage(p)} style={{ marginBottom: 12, background: `${color}14`, border: `1px solid ${color}44` }} glow>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        ...
        <div style={{ color, fontWeight: 700 }}>Manage →</div>
      </div>
    </Card>
  );
  ...
  return (
    <div style={S.page}>
      <TopBar title="🧠 Quiz Battle" onBack={() => setPage("admin_games")} />
      <div style={{ padding: "0 20px" }}>
        <Card style={{ marginBottom: 14, background: dailyStats.hardModeActive ? "rgba(255,107,107,0.1)" : "rgba(0,212,255,0.06)", border: `1px solid ${dailyStats.hardModeActive ? "#ff6b6b" : S.neonBlue}44` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <div style={{ fontWeight: 800, fontSize: 13 }}>{dailyStats.hardModeActive ? "🔴 Hard Mode Active" : "🟢 Normal Mode"}</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>{dailyStats.count} / {dailyStats.threshold} plays today</div>
          </div>
          <div style={{ height: 6, background: "rgba(255,255,255,0.08)", borderRadius: 4, overflow: "hidden", marginBottom: 8 }}>
            <div style={{ height: "100%", width: `${Math.min(100, (dailyStats.count / dailyStats.threshold) * 100)}%`, background: dailyStats.hardModeActive ? "#ff6b6b" : S.gradBlue, transition: "width 0.4s" }} />
          </div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 10 }}>{dailyStats.threshold} daily plays cross hone ke baad sirf "Hard" tagged questions milna shuru ho jaate hain. Har din midnight par reset hota hai.</div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>Threshold:</div>
            <input type="number" value={thresholdInput} onChange={e => setThresholdInput(e.target.value)} style={{ width: 90, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, color: "#fff", padding: "5px 8px", fontSize: 12 }} />
            <Btn sm onClick={saveThreshold}>Save</Btn>
          </div>
        </Card>

        <Row icon="🗂️" title="Category Management" desc={`${cats.filter(c=>c.active).length}/${cats.length} active categories`} page="admin_quiz_categories" color={S.neonBlue} />
        <Row icon="❓" title="Question Bank" desc={`${questions.filter(q=>q.active).length}/${questions.length} active questions`} page="admin_quiz_questions" color={S.neonGold} />
        <Row icon="📡" title="Match Monitoring" desc={`${liveCount} live · ${battles.length} total battles`} page="admin_quiz_matches" color={S.neonPink} />

        <Card style={{ marginTop: 4, background: "rgba(181,55,242,0.06)", border: `1px solid ${S.neonPurple}44` }}>
          <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 6 }}>🔁 Daily Question Rotation</div>
          <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.45)", marginBottom: 10 }}>
            Har din ek naya, alag mix "today's pool" ban jaata hai poore bank se — jab tak bank {poolSizeInput} se bada hai. Aaj ka pool: <b style={{ color: "#fff" }}>{todaysPool.length}</b> questions.
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>Pool size:</div>
            <input type="number" value={poolSizeInput} onChange={e => setPoolSizeInput(e.target.value)} style={{ width: 90, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, color: "#fff", padding: "5px 8px", fontSize: 12 }} />
            <Btn sm onClick={savePoolSize}>Save</Btn>
          </div>
        </Card>
      </div>
    </div>
...
export const AdminQuizCategories = ({ showToast, onBack }) => {
  ensureQuizDataSeeded();
  const [cats, setCats] = useState(DB.get("dp_quiz_categories") || []);
  const [form, setForm] = useState({ name: "", icon: "❓" });

  const save = (list) => { DB.set("dp_quiz_categories", list); setCats(list); };

  const addCategory = () => {
    const name = form.name.trim();
    if (!name) { showToast("Category naam enter karo", "error"); return; }
    const id = name.toLowerCase().replace(/\s+/g, "_");
    if (cats.some(c => c.id === id)) { showToast("Yeh category already exist karti hai", "error"); return; }
    save([...cats, { id, name, icon: form.icon.trim() || "❓", active: true }]);
    setForm({ name: "", icon: "❓" });
    showToast(`✅ Category "${name}" add ho gayi`, "success");
  };

  const toggleActive = (id) => save(cats.map(c => c.id === id ? { ...c, active: !c.active } : c));
  const remove = (id) => {
    const questions = DB.get("dp_quiz_questions") || [];
    const inUse = questions.some(q => q.categoryId === id);
    if (inUse) { showToast("Pehle is category ke questions delete/move karo", "error"); return; }
    save(cats.filter(c => c.id !== id));
  };

  return (
    <div style={S.page}>
      <TopBar title="🗂️ Quiz Categories" onBack={onBack} />
      <div style={{ padding: "0 20px" }}>
        <Card style={{ marginBottom: 16 }}>
          <div style={{ fontWeight: 800, marginBottom: 10 }}>+ Add Category</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 70px", gap: 8 }}>
            <Input label="Category Name (e.g. Cricket)" value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} icon="🗂️" />
            <Input label="Icon" value={form.icon} onChange={v => setForm(f => ({ ...f, icon: v }))} />
          </div>
          <Btn full variant="green" onClick={addCategory}>+ Add Category</Btn>
        </Card>

        <div style={{ fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,0.4)", letterSpacing: 1, marginBottom: 8 }}>ALL CATEGORIES ({cats.length})</div>
        {cats.length === 0 && <Card style={{ textAlign: "center", padding: 24, color: "rgba(255,255,255,0.3)" }}>Abhi koi category nahi</Card>}
        {cats.map(c => {
          const qCount = (DB.get("dp_quiz_questions") || []).filter(q => q.categoryId === c.id).length;
          return (
            <Card key={c.id} style={{ marginBottom: 8, opacity: c.active ? 1 : 0.5 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 14 }}>{c.icon} {c.name}</div>
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginTop: 2 }}>{qCount} question{qCount !== 1 ? "s" : ""}</div>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <Btn sm variant={c.active ? "ghost" : "green"} onClick={() => toggleActive(c.id)}>{c.active ? "Pause" : "Resume"}</Btn>
                  <Btn sm variant="danger" onClick={() => remove(c.id)}>🗑️</Btn>
                </div>
              </div>
            </Card>
          );
...
// ─── QUIZ QUESTION BANK ───────────────────────────────────────────────────────
export const AdminQuizQuestions = ({ showToast, onBack }) => {
  ensureQuizDataSeeded();
  const cats = DB.get("dp_quiz_categories") || [];
  const [questions, setQuestions] = useState(DB.get("dp_quiz_questions") || []);
  const [filterCat, setFilterCat] = useState("all");
  const [editing, setEditing] = useState(null); // question object being added/edited, or null
  const blankForm = { id: null, categoryId: cats[0]?.id || "", question: "", options: ["", "", "", ""], correct: 0, active: true, difficulty: "hard" };
  const [form, setForm] = useState(blankForm);

  // ── PDF import ──
  const [pdfBusy, setPdfBusy] = useState(false);
  const [pdfPreview, setPdfPreview] = useState(null); // { parsed: [...], unparsedCount }
  const [pdfReplaceAll, setPdfReplaceAll] = useState(true); // default: clear old bank, per admin's usual workflow
  const [pdfCategoryId, setPdfCategoryId] = useState(cats[0]?.id || "");
  const [pdfDifficulty, setPdfDifficulty] = useState("hard");
  ...
  const handlePdfFile = async (file) => {
    ...
    try {
      ...
      const { parsed, unparsedCount } = parseQuizPdfText(text);
      if (parsed.length === 0) {
        showToast("❌ PDF se koi valid MCQ nahi mila — format check karo (Q, options A-D, Answer:)", "error");
      } else {
        setPdfPreview({ parsed, unparsedCount });
        showToast(`📄 ${parsed.length} questions mile${unparsedCount ? `, ${unparsedCount} skip ho gaye` : ""} — review karke import karo`, "success");
      }
    } catch (e) {
      showToast(`❌ PDF read nahi ho paayi: ${e?.message || e}`, "error");
    }
    setPdfBusy(false);
    if (fileInputRef.current) fileInputRef.current.value = "";
  };

  const updatePreviewQ = (idx, field, value) => {
    setPdfPreview(p => ({ ...p, parsed: p.parsed.map((q, i) => i === idx ? { ...q, [field]: value } : q) }));
  };
  const updatePreviewOpt = (idx, optIdx, value) => {
    setPdfPreview(p => ({ ...p, parsed: p.parsed.map((q, i) => i === idx ? { ...q, options: q.options.map((o, oi) => oi === optIdx ? value : o) } : q) }));
  };
  const removePreviewQ = (idx) => {
    setPdfPreview(p => ({ ...p, parsed: p.parsed.filter((_, i) => i !== idx) }));
  };

  const confirmPdfImport = () => {
    if (!pdfPreview || pdfPreview.parsed.length === 0) return;
    const newQs = pdfPreview.parsed.map(q => ({
      id: tid(), categoryId: pdfCategoryId || cats[0]?.id || "gk",
      question: q.question, options: q.options, correct: q.correct,
      active: true, difficulty: pdfDifficulty, createdAt: new Date().toISOString(),
    }));
    const finalList = pdfReplaceAll ? newQs : [...newQs, ...questions];
    save(finalList);
    showToast(`✅ ${newQs.length} questions ${pdfReplaceAll ? "import ho gaye (purane sab hata diye)" : "add ho gaye"}!`, "success");
    setPdfPreview(null);
  };

  const save = (list) => { DB.set("dp_quiz_questions", list); setQuestions(list); };

  const openAdd = () => { setForm(blankForm); setEditing("new"); };
  const openEdit = (q) => { setForm({ ...q, options: [...q.options], difficulty: q.difficulty || "medium" }); setEditing(q.id); };

  const submitForm = () => {
    if (!form.categoryId) { showToast("Category select karo", "error"); return; }
    if (!form.question.trim()) { showToast("Question text daalo", "error"); return; }
    if (form.options.some(o => !o.trim())) { showToast("Sabhi 4 options bharo", "error"); return; }
    if (editing === "new") {
      const q = { id: tid(), categoryId: form.categoryId, question: form.question.trim(), options: form.options.map(o => o.trim()), correct: form.correct, active: true, difficulty: form.difficulty || "medium", createdAt: new Date().toISOString() };
      save([q, ...questions]);
      showToast("✅ Question add ho gaya", "success");
    } else {
      save(questions.map(q => q.id === editing ? { ...q, categoryId: form.categoryId, question: form.question.trim(), options: form.options.map(o => o.trim()), correct: form.correct, difficulty: form.difficulty || "medium" } : q));
      showToast("✅ Question update ho gaya", "success");
    }
    setEditing(null);
  };

  const toggleActive = (id) => save(questions.map(q => q.id === id ? { ...q, active: !q.active } : q));
  const remove = (id) => save(questions.filter(q => q.id !== id));

  const catName = (id) => cats.find(c => c.id === id)?.name || id;
  const shown = filterCat === "all" ? questions : questions.filter(q => q.categoryId === filterCat);

  return (
    <div style={S.page}>
      <TopBar title="❓ Question Bank" onBack={onBack} right={<Btn sm variant="green" onClick={openAdd}>+ Add</Btn>} />
      <div style={{ padding: "0 20px" }}>
        <Btn full onClick={() => {
          const added = adminImportHardPack();
          setQuestions(DB.get("dp_quiz_questions") || []);
          showToast(added > 0 ? `⚡ ${added} Hard/Very Hard questions add ho gaye` : "Sab pack questions pehle se bank mein hain", added > 0 ? "success" : "info");
        }} style={{ marginBottom: 10, background: "linear-gradient(135deg,#ff3d3d,#ff6b6b)" }}>⚡ 1-Click Import Hard Pack ({QUIZ_HARD_PACK.length} questions)</Btn>

        <input ref={fileInputRef} type="file" accept="application/pdf" style={{ display: "none" }}
          onChange={e => handlePdfFile(e.target.files?.[0])} />
        <Btn full onClick={() => fileInputRef.current?.click()} disabled={pdfBusy}
          style={{ marginBottom: 14, background: "linear-gradient(135deg,#00d4ff,#b537f2)" }}>
          {pdfBusy ? "📄 PDF padh rahe hain..." : "📄 Import Questions from PDF"}
        </Btn>
        <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 14, paddingBottom: 4 }}>
          <button onClick={() => setFilterCat("all")} style={{ background: filterCat === "all" ? S.gradBlue : "rgba(255,255,255,0.06)", border: "none", color: "#fff", borderRadius: 20, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>All ({questions.length})</button>
          {cats.map(c => (
            <button key={c.id} onClick={() => setFilterCat(c.id)} style={{ background: filterCat === c.id ? S.gradBlue : "rgba(255,255,255,0.06)", border: "none", color: "#fff", borderRadius: 20, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>{c.icon} {c.name}</button>
          ))}
        </div>

        {shown.length === 0 && <Card style={{ textAlign: "center", padding: 24, color: "rgba(255,255,255,0.3)" }}>Koi question nahi mila</Card>}
        {shown.map(q => {
          const diff = q.difficulty || "medium";
          const diffColor = diff === "hard" ? "#ff6b6b" : diff === "easy" ? "#34d399" : S.neonGold;
          return (
          <Card key={q.id} style={{ marginBottom: 8, opacity: q.active ? 1 : 0.5 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <div style={{ fontSize: 11, color: S.neonGold, fontWeight: 700 }}>{catName(q.categoryId)}</div>
              <Badge label={diff === "hard" ? "🔴 Hard" : diff === "easy" ? "🟢 Easy" : "🟡 Medium"} color={diffColor} />
            </div>
            <div style={{ fontWeight: 700, fontSize: 13.5, marginBottom: 6 }}>{q.question}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, marginBottom: 8 }}>
              {q.options.map((o, i) => (
                <div key={i} style={{ fontSize: 11.5, padding: "4px 8px", borderRadius: 6, background: i === q.correct ? "rgba(52,211,153,0.15)" : "rgba(255,255,255,0.05)", color: i === q.correct ? "#34d399" : "rgba(255,255,255,0.55)" }}>{i === q.correct ? "✓ " : ""}{o}</div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 6 }}>
              <Btn sm variant="ghost" onClick={() => openEdit(q)}>✏️ Edit</Btn>
              <Btn sm variant={q.active ? "ghost" : "green"} onClick={() => toggleActive(q.id)}>{q.active ? "Pause" : "Resume"}</Btn>
              <Btn sm variant="danger" onClick={() => remove(q.id)}>🗑️</Btn>
            </div>
          </Card>
          );
        })}
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title={editing === "new" ? "+ Add Question" : "✏️ Edit Question"}>
        <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 10, paddingBottom: 4 }}>
          {cats.map(c => (
            <button key={c.id} onClick={() => setForm(f => ({ ...f, categoryId: c.id }))} style={{ background: form.categoryId === c.id ? S.gradBlue : "rgba(255,255,255,0.06)", border: "none", color: "#fff", borderRadius: 20, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>{c.icon} {c.name}</button>
          ))}
        </div>
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 4 }}>Question</div>
          <textarea value={form.question} onChange={e => setForm(f => ({ ...f, question: e.target.value }))} rows={2}
            style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, color: "#fff", padding: 10, fontSize: 13, resize: "vertical", boxSizing: "border-box" }} />
        </div>
        {form.options.map((o, i) => (
          <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
            <button onClick={() => setForm(f => ({ ...f, correct: i }))} style={{ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, border: `2px solid ${form.correct === i ? "#34d399" : "rgba(255,255,255,0.25)"}`, background: form.correct === i ? "#34d399" : "transparent", color: "#fff", fontWeight: 800, cursor: "pointer" }}>{form.correct === i ? "✓" : ""}</button>
            <div style={{ flex: 1 }}>
              <Input placeholder={`Option ${i + 1}`} value={o} onChange={v => setForm(f => ({ ...f, options: f.options.map((oo, ii) => ii === i ? v : oo) }))} />
            </div>
          </div>
        ))}
        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 12 }}>Tap the circle to mark the correct answer.</div>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 6 }}>Difficulty</div>
          <div style={{ display: "flex", gap: 6 }}>
            {[["hard", "🔴 Hard", "#ff6b6b"], ["very_hard", "🔥 Very Hard", "#ff3d3d"]].map(([k, l, c]) => (
              <button key={k} onClick={() => setForm(f => ({ ...f, difficulty: k }))} style={{ flex: 1, padding: "9px 4px", borderRadius: 10, border: `1.5px solid ${form.difficulty === k ? c : "rgba(255,255,255,0.15)"}`, background: form.difficulty === k ? `${c}22` : "transparent", color: form.difficulty === k ? c : "rgba(255,255,255,0.5)", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{l}</button>
            ))}
          </div>
          <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.35)", marginTop: 6 }}>Bank mein ab sirf Hard aur Very Hard questions rakhe jaate hain.</div>
        </div>
        <Btn full variant="green" onClick={submitForm}>{editing === "new" ? "+ Add Question" : "💾 Save Changes"}</Btn>
      </Modal>

      {/* ── PDF IMPORT REVIEW ── */}
      <Modal open={!!pdfPreview} onClose={() => setPdfPreview(null)} title={`📄 Review Import (${pdfPreview?.parsed.length || 0} questions)`}>
        {pdfPreview && (
          <>
            <Card style={{ marginBottom: 12, background: "rgba(255,215,0,0.06)", border: "1px solid rgba(255,215,0,0.2)" }}>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>
                {pdfPreview.unparsedCount > 0 && <>⚠️ {pdfPreview.unparsedCount} lines parse nahi ho paayi (skip kar di) — neeche jo dikh raha hai wahi import hoga, chaho to edit kar lo.<br /></>}
                Import se pehle sab check kar lo — question text, 4 options aur ✓ sahi answer.
              </div>
            </Card>

            <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 8, paddingBottom: 4 }}>
              {cats.map(c => (
                <button key={c.id} onClick={() => setPdfCategoryId(c.id)} style={{ background: pdfCategoryId === c.id ? S.gradBlue : "rgba(255,255,255,0.06)", border: "none", color: "#fff", borderRadius: 20, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>{c.icon} {c.name}</button>
              ))}
            </div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 12 }}>👆 Yeh saare {pdfPreview.parsed.length} questions is category mein import honge.</div>

            <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
              {[["hard", "🔴 Hard", "#ff6b6b"], ["very_hard", "🔥 Very Hard", "#ff3d3d"]].map(([k, l, c]) => (
                <button key={k} onClick={() => setPdfDifficulty(k)} style={{ flex: 1, padding: "9px 4px", borderRadius: 10, border: `1.5px solid ${pdfDifficulty === k ? c : "rgba(255,255,255,0.15)"}`, background: pdfDifficulty === k ? `${c}22` : "transparent", color: pdfDifficulty === k ? c : "rgba(255,255,255,0.5)", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{l}</button>
              ))}
            </div>

            <div onClick={() => setPdfReplaceAll(r => !r)} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, padding: "10px 12px", borderRadius: 10, background: pdfReplaceAll ? "rgba(255,107,107,0.1)" : "rgba(255,255,255,0.05)", border: `1px solid ${pdfReplaceAll ? "rgba(255,107,107,0.35)" : "rgba(255,255,255,0.1)"}`, cursor: "pointer" }}>
              <div style={{ width: 20, height: 20, borderRadius: 5, border: `2px solid ${pdfReplaceAll ? "#ff6b6b" : "rgba(255,255,255,0.3)"}`, background: pdfReplaceAll ? "#ff6b6b" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0 }}>{pdfReplaceAll ? "✓" : ""}</div>
              <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.7)" }}><b style={{ color: pdfReplaceAll ? "#ff6b6b" : "#fff" }}>Replace all existing questions</b> — bank ke purane {questions.length} questions delete karke sirf yeh {pdfPreview.parsed.length} rakhe jaayenge. Uncheck karo agar purane ke saath add karna hai.</div>
            </div>

            <div style={{ maxHeight: 340, overflowY: "auto", marginBottom: 14 }}>
              {pdfPreview.parsed.map((q, i) => (
                <Card key={i} style={{ marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: 6 }}>
                    <textarea value={q.question} onChange={e => updatePreviewQ(i, "question", e.target.value)} rows={2}
                      style={{ flex: 1, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "#fff", padding: 8, fontSize: 12.5, resize: "vertical", boxSizing: "border-box" }} />
                    <Btn sm variant="danger" onClick={() => removePreviewQ(i)}>🗑️</Btn>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    {q.options.map((o, oi) => (
                      <div key={oi} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <button onClick={() => updatePreviewQ(i, "correct", oi)} style={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0, border: `2px solid ${q.correct === oi ? "#34d399" : "rgba(255,255,255,0.25)"}`, background: q.correct === oi ? "#34d399" : "transparent", color: "#fff", fontWeight: 800, fontSize: 11, cursor: "pointer" }}>{q.correct === oi ? "✓" : ""}</button>
                        <input value={o} onChange={e => updatePreviewOpt(i, oi, e.target.value)} style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, color: "#fff", padding: "5px 7px", fontSize: 11.5, boxSizing: "border-box" }} />
                      </div>
                    ))}
                  </div>
                </Card>
            ...
            <Btn full variant="green" onClick={confirmPdfImport} disabled={pdfPreview.parsed.length === 0}>
              ✅ Import {pdfPreview.parsed.length} Questions{pdfReplaceAll ? " (Replace All)" : ""}
            </Btn>
          </>
        )}
      </Modal>
...
// ─── QUIZ MATCH MONITORING ────────────────────────────────────────────────────
export const AdminQuizMatches = ({ showToast, onBack }) => {
  const [tick, setTick] = useState(0);
  const battles = [...(DB.get("dp_quiz_battles") || [])].sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""));
  const secLogs = DB.get("dp_security_logs") || [];
  const [statusFilter, setStatusFilter] = useState("all");

  const isSuspicious = (battleId) => secLogs.some(l => l.type === "quiz_collusion" && (l.details || "").includes(battleId));

  const cancel = (id) => { adminCancelQuizBattle(id, showToast); setTick(t => t + 1); };
  ...
  const shown = statusFilter === "all" ? battles : battles.filter(b => b.status === statusFilter);
  const statusColor = { waiting: S.neonGold, active: "#34d399", completed: S.neonBlue, cancelled: "rgba(255,255,255,0.4)" };

  return (
    <div style={S.page}>
      <TopBar title="📡 Match Monitoring" onBack={onBack} />
      <div style={{ padding: "0 20px" }}>
        <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 14, paddingBottom: 4 }}>
          {STATUSES.map(s => (
            <button key={s.id} onClick={() => setStatusFilter(s.id)} style={{ background: statusFilter === s.id ? S.gradBlue : "rgba(255,255,255,0.06)", border: "none", color: "#fff", borderRadius: 20, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>{s.label}</button>
          ))}
        </div>

        {shown.length === 0 && <Card style={{ textAlign: "center", padding: 24, color: "rgba(255,255,255,0.3)" }}>Koi battle nahi mila</Card>}
        {shown.map(b => {
          const suspicious = b.status === "completed" && isSuspicious(b.id);
          return (
            <Card key={b.id} style={{ marginBottom: 10, border: suspicious ? "1px solid rgba(255,61,61,0.5)" : undefined }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                <div>
                  <Badge label={b.status.toUpperCase()} color={statusColor[b.status] || S.neonBlue} />
                  {suspicious && <span style={{ marginLeft: 6 }}><Badge label="⚠️ SUSPICIOUS" color="#ff3d3d" /></span>}
                </div>
                <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.35)" }}>{timeAgo(b.createdAt)}</div>
              </div>
              <div style={{ fontSize: 13, marginBottom: 4 }}>
                <span style={{ fontWeight: 800 }}>{b.playerA?.name || "—"}</span> ({b.playerA?.score ?? 0})
                {" vs "}
                <span style={{ fontWeight: 800 }}>{b.playerB?.name || "waiting..."}</span> {b.playerB ? `(${b.playerB.score ?? 0})` : ""}
              </div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginBottom: 8 }}>
                💎{b.betAmount} entry · {b.questions?.length || 0} questions
                {b.winnerId && <> · 🏆 {b.winnerId === b.playerA?.id ? b.playerA?.name : b.playerB?.name} won</>}
              </div>
              {(b.status === "waiting" || b.status === "active") && (
                <Btn sm variant="danger" onClick={() => cancel(b.id)}>Force Cancel & Refund</Btn>
              )}
            </Card>
          );
        })}
